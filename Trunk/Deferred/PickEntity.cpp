...
void PickEntity::_getSubMeshData(int                     subMeshIndex,
...
void PickEntity::_getMeshData(const Ogre::MeshPtr     mesh,
                              size_t&                 vertex_count,
                              size_t&                 index_count,
                              const Ogre::Vector3&    position,
                              const Ogre::Quaternion& orient,
            ...
            for ( unsigned int index = 0; index < pentity->getNumSubEntities(); ++index )
            {
                ...
                for (int i = 0; i < static_cast<int>(index_count); i += 3)
                {
                    ...
                    std::pair<bool, Ogre::Real> hit = Ogre::Math::intersects(cameraRay, m_pVertexBuffer[m_pIndexBuffer[i]],
                                                                             m_pVertexBuffer[m_pIndexBuffer[i + 1]],
                                                                             m_pVertexBuffer[m_pIndexBuffer[i + 2]], true, true);
