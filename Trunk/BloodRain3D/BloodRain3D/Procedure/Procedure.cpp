...
void CProcedure::performSelection(const Vector2 &first, const Vector2 &second)
{
	...
	if ((right - left) * (bottom - top) < 0.0001)
		return;
	LogManager::getSingleton().logMessage("µ±«∞∆ ºµ„£∫°æ"+Ogre::StringConverter::toString(mStart.x)+","+Ogre::StringConverter::toString(mStart.y)+
		"°øΩ· ¯µ„£∫°æ"+Ogre::StringConverter::toString(mStop.x)+","+Ogre::StringConverter::toString(mStop.y)+"°ø");
