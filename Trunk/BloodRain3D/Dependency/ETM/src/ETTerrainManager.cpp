...
namespace ET
{
  namespace Impl
  {
	...
	void TerrainImpl::createTerrain(const TerrainInfo& info, const TileTerrainInfo& tInfo, size_t tileSize, uint maxLOD, bool vertexNormals, bool vertexTangents)
	{
		// ensure we have a viewport height set
		if (mOpt.factorC <= 0)
			OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "You need to set a valid pixel error margin.", "TerrainImpl::createTerrain");
      ...
      for (size_t i = 0; i < numTilesCol; ++i)
      {
        for (size_t j = 0; j < numTilesRow; ++j)
        {
		  ...
		  if (mSceneMgr->hasSceneNode(name))
          {
			...
			mTiles[i][j] = new Tile(name, mSceneMgr, this, mIndexHandler, mInfo, mOpt, i*(mOpt.tileSize-1), j*(mOpt.tileSize-1),true, &mTileTerrainInfo );
  ...
  void TerrainManager::createTerrain(const TerrainInfo& info, const TileTerrainInfo& tileTerrainInfo, size_t tileSize, uint maxLOD, bool vertexNormals, bool vertexTangents)
  {
