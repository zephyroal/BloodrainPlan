...
IndexData* IndexHandler::createIndexes(unsigned int lod, unsigned int neighbourState)
{
    ...
    // neighbour.
    for (unsigned int j = 0 /*north*/; j < mTileSize - 1 /*- south*/; j += step)
    {
        for (unsigned int i = 0 /*west*/; i < mTileSize - 1 /*- east*/; i += step)
        {
            // triangles
            ...
            // ttlb terrain
            int topLeft     = i * 4 + j * (mTileSize - 1) * 4;
            int topRight    = (i + step - 1) * 4 + j * (mTileSize - 1) * 4 + 1;
            int bottomLeft  = i * 4 + (j + step - 1) * (mTileSize - 1) * 4 + 2;
            int bottomRight = (i + step - 1) * 4 + (j + step - 1) * (mTileSize - 1) * 4 + 3;
