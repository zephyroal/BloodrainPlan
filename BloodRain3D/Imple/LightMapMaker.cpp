...
namespace WX
{
        ...
        void LightMapMaker::initial(void)
        {
                ...
                {
                        static const std::string msLightmapMaker("Lightmap maker");
        ...
        //--------------------------------------------------------------------------------
        void LightMapMaker::outputTexture(const std::string& fileName)
        {
                // 如果已经有lightmap了,跳过
                ...
                std::string baseName, path, outExtention;
                if (fileName.empty())
                {
                ...
                // 载入lightmap
                static const std::string TEMP_GROUP_NAME = "#TEMP#"; // 临时资源组
