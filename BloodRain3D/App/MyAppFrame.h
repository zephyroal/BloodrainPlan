...
protected:
    ...
    virtual bool setup(void)
    {
        ...
        BloodRainBites::InputContext inputContext;
        mTrayMgr = new BloodRainBites::MyTrayManager("BloodRainTrayMgr", mWindow, inputContext);
        mTrayMgr->hideFrameStats();
        mTrayMgr->showLogo(BloodRainBites::TL_BOTTOMRIGHT);
        ShowCursor(false);            // 统一使用MyGUI的鼠标指针
        mTrayMgr->hideCursor();
    ...
    virtual bool initializeShaderGenerator(SceneManager* sceneMgr)
    {
        if (RTShader::ShaderGenerator::initialize())
        {
            ...
            ResourceGroupManager::LocationList           resLocationsList = ResourceGroupManager::getSingleton().getResourceLocationList(
            ...
            for (; it != itEnd; ++it)
            {
                if ((*it)->archive->getName().find("RTShaderLib") != std::string::npos)
                {
    ...
    virtual void chooseSceneManager(void)
    {
        ...
        mSceneMgr = mRoot->createSceneManager(ST_GENERIC, "MainSMInstance");
        if (mOverlaySystem)
        {
    ...
    {
#if OGRE_PLATFORM == OGRE_PLATFORM_IPHONE
        mFrameListener = new MyFrameListener(mWindow, mCamera, true, true, true);
#else
    ...
    virtual void setupResources(void)
    {
        ...
        while (seci.hasMoreElements())
        {
            ...
            for (i = settings->begin(); i != settings->end(); ++i)
            {
                ...
                {
                    archName = std::string(macBundlePath() + "/" + archName);
                }
#endif
                recursive = false;
                if (archName[0] == '*')
                ...
                // 手动将子一级目录全部加入
                if ( recursive && "FileSystem" == typeName )
                {
                    // Get archive
                    Archive*        pArch = ArchiveManager::getSingleton().load(archName, typeName, true);
                    StringVectorPtr vec   = pArch->list(true, true);
                    for ( StringVector::iterator it = vec->begin(); it != vec->end(); ++it )
                    {
                        // Add resource location for the core shader libs.
                        ResourceGroupManager::getSingleton().addResourceLocation(archName + "/" + *it, typeName, secName, true);
                    }
                }
            }
        }
