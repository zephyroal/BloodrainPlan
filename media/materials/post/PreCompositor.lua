...
Compositor
{
	...
	Technique
	{
		...
		Target( RT_ClearGreen );
		
		Target( RT_ZWrite_Pre );
		Target( RT_Terrain_Pre );
		Target( RT_SkyDome_Pre );
		Target( RT_Sky );
		Target( RT_AlphaTest_Pre );
		...
		-- scene.def 渲染透明对象
		Target( RT_AlphaBlend_Pre );
		Target( RT_TmpTransparency_Pre );
		-- godray.def
		--[[
