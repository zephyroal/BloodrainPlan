...
namespace Hydrax {namespace Noise
                  ...
                  bool FFT::createGPUNormalMapResources(GPUNormalMapManager* g)
                  {
                      ...
                      mMaterialManager->fillGpuProgramsToPass(Technique0_Pass0, GpuProgramNames,
                                                              g->getHydrax()->getShaderMode(), EntryPoints, GpuProgramsData);
                  ...
                  void FFT::saveCfg(std::string& Data)
                  {
                      Noise::saveCfg(Data);
                  ...
                  void FFT::_calculeNoise(const float& delta)
                  {
                      ...
                      for (u = 0; u < resolution; u++)
                      {
                          for (v = 0; v < resolution; v++)
                          {
                              ...
                              realVal =
                                  positive_h0.real() * coswt - positive_h0.imag() * sinwt + negative_h0.real() * coswt - (-negative_h0.imag()) *
                                  (-sinwt),
                              imagVal =
                                  positive_h0.real() * sinwt + positive_h0.imag() * coswt + negative_h0.real() * (-sinwt) + (-negative_h0.imag()) *
                                  coswt;
                      ...
                      else
                      {
                          ...
                          return mOptions.Amplitude *
                                 (Ogre::Math::Exp(-1 / pow(k * l, 2)) / (Ogre::Math::Pow(k, 2) *
                                                                         Ogre::Math::Pow(k,
                                                                                         2))) * Ogre::Math::Pow(-dot / (k * windVelocity), kwPower_);
                      }
                  }
