...
namespace Hydrax {namespace Module
                  ...
                  std::string _SG_getNormalModeString(const MaterialManager::NormalMode& NormalMode)
                  {
                      if (NormalMode == MaterialManager::NM_VERTEX)
                      {
                          return "Vertex";
                      ...
                      if (isCreated())
                      {
                          ...
                          if (getNormalMode() == MaterialManager::NM_VERTEX)
                          {
                              ...
                              for (v = 0; v < mOptions.Complexity; v++)
                              {
                                  ...
                                  {
                                      Vertices[v * mOptions.Complexity +
                                               u].x = (static_cast<float>(v) / (mOptions.Complexity - 1)) * mOptions.MeshSize.Width;
                                      Vertices[v * mOptions.Complexity +
                                               u].z = (static_cast<float>(u) / (mOptions.Complexity - 1)) * mOptions.MeshSize.Height;
                                  }
                              }
                  ...
                  void SimpleGrid::saveCfg(std::string& Data)
                  {
                      Module::saveCfg(Data);
                  ...
                  void SimpleGrid::_performChoppyWaves()
                  {
                      ...
                      for (v = 1; v < (mOptions.Complexity - 1); v++)
                      {
                          ...
                          {
                              Vertices[v * mOptions.Complexity +
                                       u].x += Vertices[v * mOptions.Complexity + u].nx * mOptions.ChoppyStrength * Underwater;
                              Vertices[v * mOptions.Complexity +
                                       u].z += Vertices[v * mOptions.Complexity + u].nz * mOptions.ChoppyStrength * Underwater;
                          }
                      }
                  }
