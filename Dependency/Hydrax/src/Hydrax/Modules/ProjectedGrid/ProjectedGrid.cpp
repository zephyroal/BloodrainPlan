...
namespace Hydrax {namespace Module
                  ...
                  std::string _PG_getNormalModeString(const MaterialManager::NormalMode& NormalMode)
                  {
                      if (NormalMode == MaterialManager::NM_VERTEX)
                      {
                          return "Vertex";
                  ...
                  ProjectedGrid::ProjectedGrid(Hydrax*                            h,
                                               Noise::Noise*                      n,
                  ...
                  void ProjectedGrid::setOptions(const Options& Options)
                  {
                      ...
                      if (isCreated() && Options.Complexity != mOptions.Complexity)
                      {
                          ...
                          std::string MaterialNameTmp = mHydrax->getMesh()->getMaterialName();
                  ...
                  void ProjectedGrid::saveCfg(std::string& Data)
                  {
                      Module::saveCfg(Data);
                          ...
                          if (getNormalMode() == MaterialManager::NM_VERTEX)
                          {
                              ...
                              if (mOptions.ChoppyWaves)
                              {
                                  for (int i = 0; i < mOptions.Complexity * mOptions.Complexity; i++)
                                  {
                                      ...
                                      Vertices[i].y = -mBasePlane.d + mNoise->getValue(RenderingCameraPos.x + Vertices[i].x,
                                                                                       RenderingCameraPos.z + Vertices[i].z) * mOptions.Strength;
                                  }
                              }
                              else
                              {
...
/*
                  ...
                  bool ProjectedGrid::_getMinMax(Ogre::Matrix4* range)
                  {
                      ...
                      Ogre::Matrix4               invviewproj =
                          (mTmpRndrngCamera->getProjectionMatrixWithRSDepth() * mTmpRndrngCamera->getViewMatrix()).inverse();
                      ...
                      else
                      {
                          ...
                          if (height_in_plane < (mOptions.Strength + mOptions.Elevation))
                          {
                              if (underwater)
                              {
                                  mProjectingCamera->setPosition(mProjectingCamera->getRealPosition() + mLowerBoundPlane.normal *
                                                                 (mOptions.Strength + mOptions.Elevation - 2 * height_in_plane));
                              }
                              else
                              {
                                  mProjectingCamera->setPosition(mProjectingCamera->getRealPosition() + mLowerBoundPlane.normal *
                                                                 (mOptions.Strength + mOptions.Elevation - height_in_plane));
                              }
                          }
                          ...
                          if (((mBasePlane.normal).dotProduct(mTmpRndrngCamera->getDerivedDirection()) < 0.0f) ||
                              ((mBasePlane.normal).dotProduct(mTmpRndrngCamera->getDerivedPosition()) < 0.0f))
                          {
                          ...
                          {
                              Ogre::Vector3 flipped = mTmpRndrngCamera->getDerivedDirection() - 2 * mNormal *
                                                      (mTmpRndrngCamera->getDerivedDirection()).dotProduct(mNormal);
                              flipped.normalise();
                      ...
                      if (n_points > 0)
                      {
                          ...
                          {
                              if (proj_points[i].x > x_max)
                              {
                                  x_max = proj_points[i].x;
                              }
                              if (proj_points[i].x < x_min)
                              {
                                  x_min = proj_points[i].x;
                              }
                              if (proj_points[i].y > y_max)
                              {
                                  y_max = proj_points[i].y;
                              }
                              if (proj_points[i].y < y_min)
                              {
                                  y_min = proj_points[i].y;
                              }
                          }
                          ...
                          Ogre::Matrix4 invviewproj =
                              (mProjectingCamera->getProjectionMatrixWithRSDepth() * mProjectingCamera->getViewMatrix()).inverse();
