...
namespace Hydrax {namespace Module
                  ...
                  std::string _RG_getNormalModeString(const MaterialManager::NormalMode& NormalMode)
                  {
                      if (NormalMode == MaterialManager::NM_VERTEX)
                      {
                          return "Vertex";
                  ...
                  void RadialGrid::setOptions(const Options& Options)
                  {
                      ...
                      if (isCreated())
                      {
                          ...
                          if (getNormalMode() == MaterialManager::NM_VERTEX)
                          {
                              ...
                              {
                                  float r = r_scale *
                                            (mOptions.StepSizeLin *
                                             (y +
                                              1) + mOptions.StepSizeCube *
                                             Ogre::Math::Pow(y + 1, 3) + mOptions.StepSizeFive * Ogre::Math::Pow(y + 1, 5));

                                  for (x = 0; x < mOptions.Steps; x++)
                                  {
                                      Vertices[1 + y * mOptions.Steps + x].x = mOptions.Radius + r * Ogre::Math::Cos(
                                          Ogre::Math::TWO_PI * x / mOptions.Steps);
                                      Vertices[1 + y * mOptions.Steps + x].z = mOptions.Radius + r * Ogre::Math::Sin(
                                          Ogre::Math::TWO_PI * x / mOptions.Steps);
                                  }
                              }
                      ...
                      if (getNormalMode() == MaterialManager::NM_VERTEX)
                      {
                          ...
                          for (y = 0; y < mOptions.Circles; y++)
                          {
                              float r = r_scale *
                                        (mOptions.StepSizeLin *
                                         (y +
                                          1) + mOptions.StepSizeCube * Ogre::Math::Pow(y + 1, 3) + mOptions.StepSizeFive * Ogre::Math::Pow(y + 1, 5));
                  ...
                  void RadialGrid::saveCfg(std::string& Data)
                  {
                      Module::saveCfg(Data);
