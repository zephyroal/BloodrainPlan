    ...
    , mReflectionDisplacementError(0.5f)
{
    ...
    mDisableReflectionCustomNearClipPlaneRenderQueues.push_back((Ogre::RenderQueueGroupID)(Ogre::RENDER_QUEUE_SKIES_EARLY + 2));
}
...
void RttManager::_initializeRtt(const RttType& Rtt,                        const Ogre::Vector3& PlaneNormal,
                                ...
                                Ogre::RenderTargetListener* RTListener,    const std::string& MaterialScheme,
...
void RttManager::CReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
{
    ...
    // Overwater
             ...
             (mHydrax->getCamera()->getDerivedPosition().y < mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y))
    {
        mCameraPlaneDiff = mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->getPosition().y - mHydrax->getCamera()->getDerivedPosition().y +
                           mRttManager->mReflectionDisplacementError;
