                                       ...
                                       const std::string& Data)
{
    ...
    std::string ShaderModesStr[3] = {"hlsl", "cg", "glsl"};
    ...
    case NM_TEXTURE: case NM_VERTEX: case NM_RTT:
        {
            ...
            case SM_HLSL: case SM_CG:
                {
                    FragmentProgramData +=
                        std::string("float3 expand(float3 v)\n") +
                        "{\n" +
                        "return (v - 0.5) * 2;\n" +
                        "}\n\n" +
                    ...
                    if (cSun)
                    {
                        FragmentProgramData += std::string(
...
void MaterialManager::setGpuProgramParameter(const GpuProgram& GpuP, const MaterialType& MType, const std::string& Name, const Ogre::Real& Value)
{
    if (!mCreated)
    {
        return;
...
void MaterialManager::setGpuProgramParameter(const GpuProgram& GpuP, const MaterialType& MType, const std::string& Name, const Ogre::Vector2& Value)
{
    if (!mCreated)
    {
        return;
...
void MaterialManager::setGpuProgramParameter(const GpuProgram& GpuP, const MaterialType& MType, const std::string& Name, const Ogre::Vector3& Value)
{
    if (!mCreated)
    {
        return;
...
void MaterialManager::UnderwaterCompositorListener::notifyMaterialRender(Ogre::uint32 pass_id, Ogre::MaterialPtr& mat)
{
    ...
    if (mMaterialManager->mHydrax->isComponent(HYDRAX_COMPONENT_UNDERWATER_GODRAYS))
    {
        ...
        FP_Parameters->setNamedConstant("uLightDirection",
                                        (mMaterialManager->mHydrax->getMesh()->getObjectSpacePosition(mMaterialManager->mHydrax->getCamera()->
                                                                                                      getPosition()) -
        ...
        VP_Parameters->
        setNamedConstant("uCorner01",
                         mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[4] -
                         mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[5]);
        // FAR_LEFT_BOTTOM - FAR_LEFT_TOP
        VP_Parameters->
        setNamedConstant("uCorner02",
                         mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[6] -
                         mMaterialManager->mHydrax->getCamera()->getWorldSpaceCorners()[5]);
    }
