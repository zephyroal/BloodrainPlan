...
{
public:
    CoverageMap(const std::string& name, const std::string& group, uint width, uint height, int channels, bool black = true);
    ~CoverageMap();
...
CoverageMap::CoverageMap(const std::string& name, const std::string& group, uint width, uint height, int channels, bool black)
...
SplattingManager::SplattingManager(const std::string& baseName, const std::string& group, uint width, uint height, uint channels)
{
...
Image createMinimap(const Image& colourMap, const Image& lightMap)
{
    ...
    for (size_t y = 0; y < colourMap.getWidth(); ++y)
    {
        ...
        {
            ColourValue val = const_cast<Image&>(colourMap).getColourAt((uint)x, (uint)y, 0) * const_cast<Image&>(lightMap).getColourAt((uint)x,
                                                                                                                                        (uint)y,
                                                                                                                                        0);
