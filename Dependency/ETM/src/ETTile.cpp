...
const std::string&         Tile::getMovableType() const
{
    static const std::string type = "EditableTerrainTile";
    return type;
}
...
void Tile::createVertexData(size_t startx, size_t startz)
{
    ...
    for (size_t j = startz; j < endz; ++j)
    {
        ...
        for (size_t i = startx; i < endx; ++i)
        {
            ...
            if (ginfo.layer0 > mTileTerrainInfo.pixMapArray.size())
            {
                ginfo.layer0 = 0;
            }
            if (ginfo.layer1 > mTileTerrainInfo.pixMapArray.size())
            {
                ginfo.layer1 = 0;
            }
